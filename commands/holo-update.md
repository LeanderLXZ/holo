---
description: plugin 升级后的项目同步检查 — 把当前项目跟装上的 plugin 比对（`.agents/skills/` 镜像、模板新增文件 / 段 header、`CLAUDE.md` / `AGENTS.md` 头部），找 plugin 升级带来的结构性 drift。**所有检测逻辑由 `${CLAUDE_PLUGIN_ROOT}/scripts/holo_update_check.py` 单一脚本完成**，skill body 不重写规则。≥ 1 drift → 单题汇总询问 Auto-fix all / Skip all；0 drift 静默通过。CLAUDE/AGENTS 类 finding 永远只显示、不自动 merge。无参数；当前目录是否空 / 已初始化都能识别处理。不动用户已填的内容，不 git add / 不 commit。触发：/holo:update / plugin 升级了 / 同步 holo 更新 / 检查 holo 是否最新。
---
