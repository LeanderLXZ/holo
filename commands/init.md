---
description: Project skeleton initialization — thin user-entry shell that collects user answers (Step 0 language axes + Round 1 project basics + Round 2 directory classification + Round 4 doc bootstrap) and delegates all per-file landing to the shared `## Reconcile core` SOP in `commands/update.md` (mode=`"init-post-bootstrap"`). Reconcile core handles template inventory / language alignment (4-agent in-place translation for consumer files in a different `content_language`, 4-agent on-the-fly translation for plugin templates when no shipped variant matches) / NEW file copy / drift detection / 3-bucket dispatch (smart-merge for `sentinel_layout_drift` per `docs/architecture/smart-merge.md`; deterministic `--fix` for `missing_template` / `missing_section` / `missing_field` / `gitignore_missing_lines` / `lang_mirror_drift` / `agents_sync.*` / `claude_agents_lang_drift`; display-only for the rest). Three-bucket template schema preserved: REQUIRED `<...>` (filled by Step 0 + Round 1 + Step 5 substitution) / PROGRESSIVE `_(none yet — delete this marker once content is added)_` (template empty, fill as evolves) / INFERRED (Step 5 deterministic AI-infer). Optional `.agents/skills/` mirror at Step 1 pre-check. No arguments; empty directory or already initialized both handled. Never silently overwrites — markdown CONFLICTs flow through Reconcile.Step 5a smart-merge dispatch (with `take_snapshot` backup); `.gitignore` flows through Reconcile.Step 5b's deterministic `--fix` (`gitignore_missing_lines` finding category — append-only pattern sync per `ai_context/decisions.md` §Skill Implementation #14). Does not touch non-template files, does not git add, does not commit. Triggers: /holo:init / initialize project / install skeleton / create an empty project.
---
