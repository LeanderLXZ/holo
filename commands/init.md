...
**Sentinel notice in entry files (CLAUDE.md / AGENTS.md)**: the plugin templates' `CLAUDE.md` + `AGENTS.md` carry an inline "Plugin-managed sections (sentinel contract)" blockquote inside their preamble sentinel block (added by T-SECTION-VERSION-SENTINEL Phase 4 — lives inside `<!-- holo:section start --> ... <!-- holo:section end -->` so it counts as plugin canonical content and is overwritten on upgrade if a consumer deletes it). The notice explains the `<!-- holo:heading -->` ownership contract and the **no-opt-out rule** (deleting a marker no longer detaches a section; `/holo:update` surfaces it as `unmarked_heading` / `unmarked_section` drift and `--fix` re-adds it; consumers needing a permanently customized body fork the plugin — per `ai_context/decisions.md` §Skill Implementation #20). This step needs NO new write-back path: the standard template copy above brings the notice through into the consumer's entry files. The notice is byte-identical across `CLAUDE.md` ↔ `AGENTS.md` per the existing entry-file sync contract.
