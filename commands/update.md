---
description: Project sync check after plugin upgrade — thin user-entry shell around the shared `## Reconcile core` SOP defined in this same file. **All file-update logic lives in `## Reconcile core`** (6 sub-steps: template inventory → language alignment → NEW file copy → drift detection via `${CLAUDE_PLUGIN_ROOT}/scripts/holo_update_check.py` → 3-bucket dispatch → return); `/holo:update` (`mode="update"`) and `/holo:init` Step 4 (`mode="init-post-bootstrap"`) both flow through it so the per-file landing protocol has a single source of truth. Conflict-triggering findings (`sentinel_layout_drift` — any sub_shape) → smart-merge dispatch (4 sub-agent parallel + three-layer ask + `take_snapshot` backup, per `docs/architecture/smart-merge.md`). Other fixable findings (`missing_template` / `missing_section` / `missing_field` / `gitignore_missing_lines` / `agents_sync.*` / `claude_agents_lang_drift`) → deterministic `--fix`. Display-only by design: `claude_agents.unexpected_diffs` / `lang_mirror_drift` (per-language variant content is human translation work; auto-overwrite would destroy translations) / `legacy_skip_marker` / `missing_l1_directive` / `l1_directive_drift`. File-body language mismatch is preprocessed by Reconcile.Step 2b (4-agent in-place chain), not a smart-merge trigger. 0 drift passes silently. No arguments; preserves user-territory content; does not git add, does not commit. Triggers: /holo:update / plugin upgraded / sync holo update / check whether holo is up to date.
---
