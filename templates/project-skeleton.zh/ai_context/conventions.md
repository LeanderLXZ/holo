...
`logs/change_logs/` 对每次改动采用三时间点契约：
PRE / POST / REVIEW — 一个日志文件覆盖一次完整改动的生命周期。
...
- 无 PRE 日志 → 不准开始改文件。
- 早于本约定的旧式单时间点日志保持原样；不要回溯改写。

当项目使用内置的 `/go` 和 `/post-check` 时，这两个 skill 拥有
确切的日志格式；以它们的定义为准。
...
<项目相关。记录本项目使用的 ID 方案与语言约定 — 例如 ID 前缀模式、
零填充宽度、哪些文件保持哪种语言、JSON 字段的语言规则。在真正
有约定生效之前保持空白。>

## Data Separation

<项目相关。记录不可逾越的硬 schema 门控与数据边界 — 例如用户数据
与权威数据的隔离、哪些字段是 write-once 与哪些可演化、哪些目录的
数据不可流入哪些目录。在真正设定边界之前保持空白。>

## Git

<项目相关。记录分支模型、push 策略与 do-not-commit 规则。若项目
采用非平凡的多分支流，在此描述；否则留一句 "单分支
`main`；commit 小颗粒，push 勤"。>
