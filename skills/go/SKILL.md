---
name: go
description: Heavyweight landing path — pick only when: (a) cross-file alignment needed; (b) want /post-check re-validation against PRE intent baseline; (c) dirty tree / branch switch / worktree isolation; (d) smoke or data-contract validation before commit; (e) planned set ≥ 3 files. Otherwise prefer /do. 11-step flow: load config → ask work location → PRE log → doc authoring → implementation → smoke → cross-file alignment + todo_list maintenance → multi-line review → POST log → commit → wrap-up. Step 1 is the only Q before Step 10; Step 2 PRE log is /post-check's intent baseline (no PRE → no file changes allowed). Cross-branch sync → /forward. $ARGUMENTS = focus (optional). Triggers: go / full /go / this change needs review / spans multiple modules / heavyweight landing.
---
