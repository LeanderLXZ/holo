---
name: run-prompt
description: Load and execute a specified prompt file as the current task — $ARGUMENTS = path or stem (path read directly; stem fuzzy-matched across prompts/ tree; 0 matches fail loudly + list directory / ≥ 2 matches list all / 1 match used). Single outer fenced block → take inner content; otherwise take whole file. Print resolved path → take over execution treating prompt as user instruction. Required fields not supplied via $ARGUMENTS → list them first. Reads only; no edits / commit / push. Triggers: run prompt / run-prompt <name> / run <prompt> / load <prompt>.
---
