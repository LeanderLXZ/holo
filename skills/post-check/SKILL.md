---
name: post-check
description: Focused review after /go — two tracks in parallel: track 1 reconciles PRE log's planned action list + validation criteria against actual diff; track 2 expands to files outside the plan to find cross-file conflicts / ambiguity / bugs. Mandatory PRE log read (under logs/change_logs/) as intent baseline; may dispatch sub-agents in parallel for spec / impl / risk / structure lines. $ARGUMENTS = log slug (defaults to latest). Read-only except summary written back to log. Full-repo audit → /full-review. Triggers: re-confirm this change / after-check / review after /go completes / post-check.
---
