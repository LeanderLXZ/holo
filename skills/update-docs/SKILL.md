---
name: update-docs
description: Land conversation narrative into ai_context/ + docs/ files — semantic match decides which file + section each just-discussed point belongs to, composes patches respecting the PROGRESSIVE marker contract, surfaces lockstep pairs from conventions.md §Cross-File Alignment, previews patches in one batched <ask tool> (Confirm / Tweak / Cancel), applies on confirm, reminds /commit to persist. $ARGUMENTS = focus (file path / section name / topic keyword; optional). Patches outside ai_context/ + docs/ rejected. Sibling of /todo-add for prose. Triggers: /update-docs / update docs / record discussion in ai_context / land discussion into docs.
---
